package {
	...
	public class ButterflyCatcher002 extends MovieClip {
		...
		private var playingClips:Array = new Array();
		

		public function ButterflyCatcher002(){
		...
		*/
		...
		private function updateTracks(e:Event) {
			...
			//trace("track1Volume: " + percentControlled);
			//trace("track2Volume: " + (1-percentControlled));
		...
		private function updateSoundBuffer(event:Event):void {
			...
			/*if(soundPlaying == false) {
				//****START AUDIO CODE****				
				trace("start sound -> soundBuffer.length = " + Globals.soundBuffer.length);
				clipReq = new URLRequest(Globals.soundBuffer.pop());
				clip = new Sound();
				clip.load(clipReq);
				soundPlaying = true;
				
				clipHandler = clip.play();
				
				clipHandler.addEventListener(Event.SOUND_COMPLETE, soundOver);
				//****END AUDIO CODE****				
			}*/
			for each(var req in soundBuffer) {
			var newClip = new Sound();
			newClip.load(new URLRequest(req));
			var newClipHandler:SoundChannel = newClip.play();
			playingClips.push(newClip);
			playingClips.push(newClipHandler);
			newClipHandler.addEventListener(Event.SOUND_COMPLETE, soundOver2);
			soundBuffer.pop();
			}
		}
		...
		public function soundOver2(e:Event):void {
			for(var i=0;i<playingClips.length; i++) {
				if(playingClips[i] == e.target) {
